...
def import_seed(seed_path: str, force: bool = False):
    ...
    for item in items:
        ...
        item.setdefault("taxon", "")
        item.setdefault("scope", "domain")
        item.setdefault("applies_when", "")
        item.setdefault("does_not_apply_when", "Do not treat seed knowledge as universal without review.")
        item.setdefault("evidence_level", "local_rule" if item.get("reasoning") else "raw_observation")
        item.setdefault("promotion_state", "candidate")
        item.setdefault("project_path", None)
        item.setdefault("source_lineage", {"kind": "manual_seed", "ref": str(path)})
        item.setdefault("evidence_excerpt", "")
        item.setdefault("counterexamples", [])
        item.setdefault("conflicts_with", [])
        item.setdefault("review_required", 1)
        item.setdefault("safe_to_generalize", 0)
